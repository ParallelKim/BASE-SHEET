...
스튜디오 재생: 곡을 고르면 `review_from`(있지 13마디)부터. 미리듣기 WAV가 있으면 재생 버튼이 그걸 쓰고 롤·줄이 같은 시간. 피아노롤을 누르면 시크.

스튜디오 악보 비교: 같은 MIDI를 피아노롤, OSMD 오선, alphaTab 오선+탭으로 바꿔 본다. 악보를 그리기 전에 짧은 음정 흔들림은 빼고 같은 음은 한 음표로 잇는다. 피아노롤 MIDI는 그대로다. MuseScore는 MIDI를 받아 데스크톱에서 연다. alphaTab 줄·프렛은 표준 튜닝 추정이다.

전사 MIDI: 앞뒤가 같은 음고일 때 그 사이에 낀 0.12초 이하의 다른 음은 쓰기 전에 뺀다. 같은 음의 반복은 붙이지 않는다.

전사 MIDI: 옥타브 선택은 위·아래를 같은 기준으로 본다. 맨 아래 옥타브에 적힌 음도, 그 근음과 홀수 배음이 없고 한 옥타브 위가 본체이면 올린다. 이웃 유무나 음 길이로 예외를 두지 않는다. 이웃 접기는 이 선택 앞에서 끝나고, 스펙트럼 결과가 다시 접히지 않는다.
